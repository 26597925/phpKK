...
class Cache {
	 ...
	 */
    public function __construct( $cache = 'app.db_cache' ) {
		if( $cache ){
			$this->cache = $cache;
		}
		$cache = App::get("app.cache");
		$this->config = $cache[$this->cache];
		if( empty($this->config) || !isset($this->config['app.cache_type']) ) {
	...
	public function __call($method, $args) {
		if( !isset(self::$objArr[$this->cache]) ){		
			$cacheDriver = __NAMESPACE__.'\cache\\' . ucfirst( $this->config['app.cache_type'] ).'Driver';
