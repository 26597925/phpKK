...
class Model {
     ...
     * @var string
     */
    protected $database = 'app.default';
     ...
     */
    public function __construct( $database = 'app.default' ) {
        if( $database ){
            $this->database = $database;
        }
        $this->config = App::get($this->database);
        if( empty($this->config) || !isset($this->config['app.db_type']) ) {
            throw new \Exception($this->database.' database config error', 500);
        }
        $this->table = (null==$this->trueTable) ? $this->config['app.db_prefix'].$this->table : $this->trueTable;
    ...
    public function query($sql, $params = array()) {
        ...
        $sql = str_replace('{pre}', $this->config['app.db_prefix'], $sql);
    ...
    public function table($table, $ignorePre = false) {
        $this->options['table'] = $ignorePre ? $table : $this->config['app.db_prefix'] . $table;
        return $this;
    }
    ...
    public function join($join, $way='inner') {
        $join = str_replace('{pre}', $this->config['app.db_prefix'], $join);
    ...
    public function cache($expire = 1800) {
        $cache = new Cache($this->config['app.db_cache']);
        $cache->proxyObj = $this;
    ...
    protected function getDb() {
        if( empty(self::$objArr[$this->database]) ){
            $dbDriver = __NAMESPACE__.'\db\\' . ucfirst( $this->config['app.db_type'] ).'Driver';
