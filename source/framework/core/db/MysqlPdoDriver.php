...
class MysqlPdoDriver implements DbInterface {
	...
	public function query($sql, array $params = array()) {
		...
		$err = $sth->errorInfo();
		App::exception('dbException', array($this->getSql(), $err[2]));
	...
	protected  function _connect($isMaster = true) {
		...
		$error = '';
		foreach($dbArr as $db) {
			$dsn = "mysql:host={$db['app.db_host']};port={$db['app.db_port']};dbname={$db['app.db_name']};charset={$db['app.db_charset']}";
			try{
				$pdo = new \PDO($dsn, $db['app.db_user'], $db['app.db_pwd']);
				break;
		...
		$pdo->exec("set names {$db['app.db_charset']}");
		return $pdo;
	}
