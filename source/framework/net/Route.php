...
class Route {
    ...
    public function __construct($pattern, $callback, $methods, $pass, $group) {
    ...
    public function matchUrl($url, $case_sensitive = false) {
        ...
        if (preg_match('#^'.$regex.'(?:\?.*)?$#'.(($case_sensitive) ? '' : 'i'), $url, $matches)) {
			...
			$vals = array();
			foreach ($ids as $k => $v) {
                $vals[$k] = (array_key_exists($k, $matches)) ? urldecode($matches[$k]) : null;
            }
			...
			if(empty($this->params))
			{
				$this->request->data->setData($vals);
				$this->params[] = $this->request;
			}
